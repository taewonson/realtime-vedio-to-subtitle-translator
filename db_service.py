...
class DBService:
    ...
    def __init__(self, key_path):
        ...
        # GCP 서비스 어카운트 키 파일을 읽어와 Firestore 클라이언트를 초기화함
        try:
            if os.path.exists(key_path):
                ...
                self.db_client = firestore.Client(
                    ...
                    database="capstonevoca" # 사용할 Firestore 데이터베이스 이름 지정
                )
    ...
    def translate_and_save_word(self, user_id, word, provided_lang_name):
        """파이(LCD)에서 보낸 단어를 GCP Translate API로 한국어로 번역한 뒤 Firestore 단어장에 저장"""
        ...
        try:
            ...
            # 원본 언어를 자동 감지한 경우 감지된 언어 이름을, 아니면 사용자가 선택한 언어 이름을 사용
            lang_name = code_to_name.get(detected_code, detected_code.upper()) if provided_lang_name == "원본" else provided_lang_name
            ...
            print(f"🗳️ DB 저장 완료 [{user_id}] -> {word} : {translated_word}")
        except Exception as e:
